...
//! A piece table provides efficient methods for inserting and removing elements
//! sequentially, intended for use as the underlying data structure in a text editor.
//!
//! The piece table stores a read-only reference to a source (if one is provided)
//! and stores inserted elements in an append-only vector. A table of pieces pointing
//! either to the source or add-buffer is maintained, and these pieces are manipulated
//! when inserting and removing text.
//!
//! Asymptotics in the following are based on `p`, the number of pieces, where `p`
//! should be strictly smaller than the number of elements when used as intended.
...
impl<'a, T: 'a> PieceTable<'a, T> {
    ...
    /// Scattered operations result in pieces being added; the created piece table will be able
    /// to store `piece_capacity` of these before reallocating.
    ...
    fn make_iter(&'a self, idx: usize) -> Iter<'a, T> {
        let (piece_idx, norm_idx) = match self.idx_to_location(idx) {
            ...
            PieceMid(piece_idx, norm_idx) | PieceTail(piece_idx, norm_idx) => (piece_idx, norm_idx),
            EOF => {
                return Iter {
                    table: self,
                    piece_idx: self.pieces.len(),
                    it: self.adds[0..0].iter(),
                };
            }
        };
    ...
    fn raw_insert(&mut self, idx: usize, item: T) {
        ...
        match self.idx_to_location(idx) {
            ...
            PieceMid(piece_idx, norm_idx) | PieceTail(piece_idx, norm_idx) => {
                ...
                // Replaced unsafe push_all_at with safe splice
                self.pieces.splice(
                    (piece_idx + 1)..(piece_idx + 1),
                    [
                        Piece {
                            start: item_idx,
                            length: 1,
    ...
    /// Appends an element to the back efficiently in constant time.
    pub fn push(&mut self, value: T) {
        let reuse = self
            .pieces
            .last()
            .map_or(false, |last| {
                last.buffer == Add && last.start + last.length == self.adds.len()
            });
...
impl<'a, T> std::iter::FromIterator<T> for PieceTable<'a, T> {
    fn from_iter<I: IntoIterator<Item = T>>(iterable: I) -> PieceTable<'a, T> {
        let mut table = PieceTable::new();
        table.adds = iterable.into_iter().collect();
